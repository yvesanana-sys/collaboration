...
WRITE A COMPLETE SLEEP BRIEF. 

CRITICAL: Output ONLY raw JSON. Zero prose. Zero markdown. Zero comments.
Start your response with {{ and end with }}. Nothing before or after.
Use null (not None), true/false (not True/False).
All string values must use double quotes.

{{
  "portfolio_assessment": "<2 sentences max>",
  ...
  "stocks": {{
    "<SYMBOL>": {{
      ...
      "time_review_hrs": <number or null>,
      "circuit_breaker": <price 20pct below entry or null>,
      "bot_approved_action": <"sell_full"|"sell_half"|null>
    }}
  }},
  "crypto": {{
    "<SYMBOL>": {{
  ...
  "bot_may_execute": ["<explicit action 1>"],
  "bot_may_NOT_execute": ["sell any unlisted position", "buy any new asset"]
}}
"""
    return prompt
...
def parse_sleep_brief(raw_response: str) -> dict | None:
    ...
    Very robust — handles all Claude output variations:
      - Markdown fences (``` json, ```json, ``` )
      - Trailing commas before } or ]
      - Single quotes instead of double quotes
      - Python literals (True/False/None → true/false/null)
      - Prose before or after the JSON block
      - Truncated responses (tries partial parse)
      - Comments (// ...) inside JSON
    Returns None only if completely unparseable.
    """
    import re, json
    ...
    def _clean(text: str) -> str:
        # 1. Strip markdown fences (``` json, ```json with space, ```)
        ...
        # 5. Single quotes → double quotes (careful with apostrophes)
        # Only replace ' used as JSON string delimiters
        text = re.sub(r"(?<![a-zA-Z])'([^']*)'(?![a-zA-Z])", r'"\1"', text)
        return text.strip()

    def _try_parse(text: str) -> dict | None:
        """Find outermost JSON object and parse it."""
        s = text.find('{')
        if s < 0:
            return None
        # Try from each closing brace backwards for truncated responses
    ...
    # ── Attempt 2: extract just the JSON block between first { and last } ──
    s = cleaned.find('{')
    e = cleaned.rfind('}')
    if s >= 0 and e > s:
        fragment = cleaned[s:e+1]
        # Aggressive trailing comma fix
    ...
    # ── Attempt 3: try to extract minimal valid subset ─────────
    # If Claude output prose + JSON, find the last {...} block
    for match in reversed(list(re.finditer(r'\{', cleaned))):
        s = match.start()
        fragment = cleaned[s:]
        e = fragment.rfind('}')
        if e > 0:
            candidate = re.sub(r',\s*([}\]])', r'\1', fragment[:e+1])
            try:
                result = json.loads(candidate)
