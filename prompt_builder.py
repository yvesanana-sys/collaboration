...
class PromptBuilder:
    ...
    def build_r1(self, equity, cash, positions, pos_details,
        ...
        pnl_today_pct = 0.0
        try:
            pnl_today_pct = (equity - 55) / 55  # rough daily pnl estimate
        except Exception:
            pass
...
Plain text 180 words."""
    ...
    def build_claude_system(self):
        ...
        base = "You are Claude, a disciplined quantitative trader. Respond with ONLY valid JSON under 500 chars."
        if persona:
    ...
    def build_grok_system(self):
        ...
        base = "You are Grok, a momentum trader with Twitter/X access. Respond with ONLY valid JSON under 500 chars."
        if persona:
            return f"{base} {persona}"
        return base
...
INDICATORS: {chart_section[:450]}"""
    ...
    def _build_task_instructions(self, mode, near_stop, near_tp,
        ...
        """
        base_json = ('JSON: {"strategy_name":"name","market_thesis":"brief",'
                     '"proposed_trades":[{"action":"buy/sell/short","symbol":"TICK",'
                     '"notional_usd":15.0,"confidence":85,"direction":"long/short",'
                     '"signals":["s1","s2","s3"],"rationale":"brief",'
                     '"politician_signal":false,"ipo_signal":false}],'
                     '"collaborative_candidates":[{"symbol":"TICK","reason":"brief","confidence":90}],'
                     '"bearish_watchlist":["tickers"]}')
