...
# Persisted to disk so Railway restarts don't wipe history
TRADE_HISTORY_FILE = "/app/trade_history.json"
...
def _save_trade_history(history: list):
    """Save trade history to disk after every new entry."""
    for path in [TRADE_HISTORY_FILE, "./trade_history.json"]:
        try:
...
def record_trade(action, symbol, qty, price, notional, owner,
    ...
    Persists to disk so history survives Railway restarts.
...
Both AIs agree on this plan. Plain text 200 words."""
        ...
        # ── Always sleep after afterhours completes ────────────
        # AIs woke for the 4pm review — now put them back to sleep.
        # Without this, ai_sleeping stays False and snapshot shows
        # "✅ AWAKE" all night even though no AI calls are running.
