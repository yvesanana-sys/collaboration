...
# ── STOCK-ONLY EXECUTION FILTER ──────────────────────────────
# Prevents crypto tickers (USDT pairs) from entering stock trading cycle.
# Claude/Grok sometimes suggest BTC/ETH crosses when analyzing crypto spillover;
# this validator silently filters them out before Alpaca execution.
STOCK_EXEC_FORBIDDEN_PATTERNS = [
    "USDT",     # Crypto stablecoin pairs
    "USDC",     # USD Coin pairs
    "BUSD",     # Binance USD pairs
    "TUSD",     # True USD pairs
]

def is_crypto_ticker(symbol: str) -> bool:
    """Return True if symbol contains crypto stablecoin patterns."""
    if not isinstance(symbol, str):
        return False
    upper = symbol.upper()
    return any(pattern in upper for pattern in STOCK_EXEC_FORBIDDEN_PATTERNS)
