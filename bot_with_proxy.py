...
def get_volatility_targets(symbol: str, atr: float, price: float,
                           entry: float, timeframe: str = "daily") -> dict:
    """
    Compute volatility-aware profit targets and stops based on ATR.
    Goal: capture 5-10% profit daily, 15-25% weekly, 40-60% monthly.

    ATR-based approach:
      - Stop  = entry - 1.5× ATR  (tight enough to cut losers fast)
      - TP1   = entry + 2.0× ATR  (first target — lock in partial profit)
      - TP2   = entry + 3.5× ATR  (main target — full exit)
      - Trail = activated at TP1, trail by 1.0× ATR

    Returns dict with stop, tp1, tp2, trail_distance, expected_rr.
    """
    if not atr or atr <= 0 or not price or price <= 0:
        # Fallback to percentage-based if no ATR
        return {
            "stop":           round(entry * 0.96, 2),   # -4%
            "tp1":            round(entry * 1.05, 2),   # +5%
            "tp2":            round(entry * 1.08, 2),   # +8%
            "trail_distance": round(entry * 0.02, 2),   # 2%
            "expected_rr":    2.0,
            "atr_used":       False,
        }
...
def get_portfolio_performance_vs_targets(equity: float, daily_pnl: float,
                                          positions: list) -> dict:
    """
    Check how today's performance compares to daily/weekly/monthly targets.
    Target: 5% daily, 15% weekly, 40% monthly on trading pool.

    Returns summary with status and guidance for AIs.
    """
    pool          = get_trading_pool(equity)["trading"]
    daily_target  = pool * 0.05   # 5%
    weekly_target = pool * 0.15   # 15%
    monthly_target= pool * 0.40   # 40%

    daily_pct  = round(daily_pnl / max(pool, 1) * 100, 2)
    daily_hit  = daily_pnl >= daily_target

    # Estimate weekly from daily (rough)
    weekly_pnl_est  = daily_pnl * 5   # assumes average day
    weekly_hit      = weekly_pnl_est >= weekly_target

    # Open position unrealized
    open_pnl = sum(float(p.get("unrealized_pl", 0)) for p in positions)

    status = "🟢 ON TRACK" if daily_hit else (
             "🟡 BEHIND"   if daily_pct > 0 else "🔴 LOSING")
    ...
    return {
        ...
        "guidance": (
            f"Need ${daily_target - daily_pnl:.2f} more today to hit 5% daily target"
            if not daily_hit else
            f"✅ Daily 5% target hit! Lock in gains, avoid giving back profits"
        ),
    }
...
def check_pdt_safe(symbol: str, equity: float = 55.0) -> tuple:
    """
    Check if selling this symbol is PDT-safe.
    Returns (safe: bool, reason: str)

    PDT rule: accounts < $25,000 → max 3 day trades per 5 business days.
    Violation = account locked for 90 days.
    """
    if equity >= 25000:
    ...
    if used >= 3:
        return False, (f"PDT LIMIT REACHED: {used}/3 day trades used this week. "
                       f"Cannot sell {symbol} today — would trigger 90-day restriction.")

    remaining = 3 - used
    return True, f"day trade OK — {remaining} of 3 remaining this week"


def run_pdt_hold_council(symbol: str, pos: dict,
                          ask_claude_fn, ask_grok_fn) -> dict:
    """
    When PDT blocks a sell, both AIs run a collaborative multi-day
    price simulation to decide:
      - Hold 1 day? 2 days? 3 days?
      - What is the projected plateau price?
      - Where to set the exit limit?
      - Where to set the stop to protect if wrong?

    Both AIs must AGREE on the exit plan. If they disagree, they
    negotiate a conservative middle ground.

    Returns a hold_plan dict stored in shared_state for the bot to follow.
    """
    try:
...
def get_pdt_decision(symbol: str, equity: float,
    ...
    """
    Full PDT-aware hold/sell decision with projection analysis.

    When PDT limit would be hit, decides whether to:
    - HOLD overnight if projection is still bullish
    - FORCE SELL anyway if projection is bearish (stop-loss priority)
    - TRAIL STOP to protect profit while holding overnight

    Returns:
        action:      'sell' | 'hold_overnight' | 'trail_stop'
        reason:      explanation string
        new_stop:    updated stop price if trailing
        pdt_used:    current day trade count
        pdt_left:    remaining day trades
        proj_bias:   bullish/bearish/neutral from projection engine
        expected_tomorrow: projected price range for tomorrow
    """
    used      = shared_state.get("day_trade_count", 0)
...
def smart_sell(symbol, reason, pos):
    ...
    try:
        ...
        log(f"❌ Sell {symbol}: {e}")
        err_str = str(e)
        # Track failed sells — if 403 persists, wake AIs
        if "403" in err_str or "Forbidden" in err_str or "halted" in err_str.lower():
            ...
            log(f"⚠️ {symbol} sell forbidden — attempt {fails}/3 "
                f"(halted/restricted?)")
            if fails >= 3:
                # Can't sell after 3 attempts — wake AIs, they need to handle this
                log(f"🚨 {symbol} UNSELLABLE after {fails} attempts — waking AIs")
                ai_wake(f"EMERGENCY: {symbol} cannot be sold (403 Forbidden) "
                        f"— position may be halted or restricted")
...
JSON: {{"refined_trades":[{{"action":"buy/sell/short","symbol":"NVDA","notional_usd":15.0,"confidence":85,"direction":"long/short","signals":["s1","s2","s3"],"rationale":"brief","owner":"claude"}}]}}"""
...
JSON: {{"refined_trades":[{{"action":"buy/sell/short","symbol":"NVDA","notional_usd":15.0,"confidence":85,"direction":"long/short","signals":["s1","s2","s3"],"rationale":"brief","owner":"grok"}}]}}"""
...
def execute_trades(final_trades, cash, pos_symbols, open_count, final_plan, features):
    ...
    for trade in final_trades:
        ...
        # Crypto (BTCUSDT, ETH/USD etc.) must ONLY go through Binance.US
        # MSTR and COIN are stocks — allowed. BTC/ETH direct pairs are not.
