...
shared_state = {
    ...
    # ── Streak & regime tracking ─────────────────────────
    "consecutive_wins":     0,       # current win streak
    "consecutive_losses":   0,       # current lose streak
    "cooldown_until":       None,    # no new trades until this datetime (after 2 losses)
    "last_regime":          "NEUTRAL", # last detected market regime
    "win_streak_bonus":     1.0,     # size multiplier from streaks (0.5-1.5)
    "grok_intel_history":   [],      # last 5 Grok fingerprints for freshness check
    "grok_last_changed":    None,    # timestamp when Grok intel last changed
}
...
def record_trade(action, symbol, qty, price, notional, owner,
    ...
    # ── Streak tracking: update win/lose streaks on closes ──────
    if action in ("take_profit", "trail_stop", "thesis_exit", "time_stop", "stop_loss") and pnl_usd is not None:
        if pnl_usd > 0:
            shared_state["consecutive_wins"]   += 1
            shared_state["consecutive_losses"]  = 0
            # Win streak bonus: after 3 wins → 1.2x size; after 5 wins → 1.4x
            wins = shared_state["consecutive_wins"]
            shared_state["win_streak_bonus"] = min(1.4, 1.0 + (wins // 3) * 0.2)
            log(f"🏆 Win streak: {wins} | size bonus: {shared_state['win_streak_bonus']:.1f}x")
        else:
            shared_state["consecutive_losses"] += 1
            shared_state["consecutive_wins"]    = 0
            shared_state["win_streak_bonus"]    = 1.0  # reset size bonus
            losses = shared_state["consecutive_losses"]
            if losses >= 2:
                # 2 consecutive losses → 12hr trading cooldown
                from datetime import timedelta as _td
                cooldown_end = datetime.now() + _td(hours=12)
                shared_state["cooldown_until"] = cooldown_end.isoformat()
                log(f"⛔ LOSE STREAK {losses} — trading cooldown until {cooldown_end.strftime('%H:%M ET')}")
            elif losses == 1:
                log(f"📉 Loss recorded. {2-losses} more triggers cooldown.")
...
def alpaca(method, path, body=None, base=None, params=None):
    headers = {
        "APCA-API-KEY-ID": ALPACA_KEY,
        "APCA-API-SECRET-KEY": ALPACA_SECRET,
        "Content-Type": "application/json",
    }
    res = requests.request(method, (base or BASE_URL) + path, headers=headers, json=body, params=params)
    res.raise_for_status()
    return res.json()
...
def compute_volume_profile(bars: list, lookback: int = 20) -> dict:
    """
    3-4 week Price vs Volume analysis.
    Uses existing OHLCV bars — ZERO extra API calls.
    
    Returns a dict with predictive signals:
      phase:          accumulation | distribution | coiling | trending_up | trending_down | climax_top | exhaustion_bottom
      obv_slope:      float — positive = money flowing in, negative = out
      obv_divergence: bullish | bearish | confirming | neutral
      vwap_20d:       float — 20-day volume-weighted average price (fair value)
      price_vs_vwap:  above | below | at
      buy_sell_ratio: float — >1.2 buyers dominate, <0.8 sellers dominate
      vol_trend:      expanding | contracting | climax | normal
      prediction:     str — 1-line forecast for AI
      conf:           int — 0-100 confidence in prediction
    """
    ...
    highs   = [float(b.get("h", b.get("high", closes[i]))) for i, b in enumerate(bars)]
    lows    = [float(b.get("l", b.get("low",  closes[i]))) for i, b in enumerate(bars)]
    ...
    # OBV slope via linear regression (positive = rising)
    xs    = list(range(n))
    x_avg = sum(xs) / n
    y_avg = sum(obv_series) / n
    num   = sum((xs[i] - x_avg) * (obv_series[i] - y_avg) for i in range(n))
    den   = sum((xs[i] - x_avg) ** 2 for i in range(n))
    obv_slope = round(num / den, 0) if den != 0 else 0.0

    # Normalize slope as % of average volume for comparability
    avg_vol = sum(vols) / n if n > 0 else 1
    obv_slope_norm = round(obv_slope / avg_vol * 100, 1)  # % of avg vol per day

    # Price slope (same method)
    yp    = closes
    yp_avg = sum(yp) / n
    num_p  = sum((xs[i] - x_avg) * (yp[i] - yp_avg) for i in range(n))
    ...
    if price_rising and obv_rising:
        obv_div = "confirming"        # both up = healthy trend
    elif price_rising and not obv_rising:
        obv_div = "bearish"           # price up, money leaving = danger
    elif not price_rising and obv_rising:
        obv_div = "bullish"           # price down, money entering = opportunity
    else:
        obv_div = "neutral"           # both flat/down = weak trend

    # ── 2. VWAP over lookback period (20-day fair value) ─────
    total_vol   = sum(vols)
    vwap_20d    = 0.0
    if total_vol > 0:
        typical_prices = [(h + l + c) / 3 for h, l, c in zip(highs, lows, closes)]
        vwap_20d = round(sum(tp * v for tp, v in zip(typical_prices, vols)) / total_vol, 4)

    current_price = closes[-1]
    vwap_gap_pct  = round((current_price - vwap_20d) / vwap_20d * 100, 2) if vwap_20d > 0 else 0
    if vwap_gap_pct > 1.5:
        price_vs_vwap = "above"    # trading above fair value (expensive)
    elif vwap_gap_pct < -1.5:
        price_vs_vwap = "below"    # trading below fair value (cheap)
    else:
        price_vs_vwap = "at"       # at fair value

    # ── 3. Buy/Sell Volume Ratio ──────────────────────────────
    buy_vol  = sum(vols[i] for i in range(n) if closes[i] >= closes[i-1] if i > 0)
    sell_vol = sum(vols[i] for i in range(n) if closes[i] <  closes[i-1] if i > 0)
    buy_sell_ratio = round(buy_vol / sell_vol, 2) if sell_vol > 0 else 1.5

    # ── 4. Volume Trend (expanding/contracting/climax) ────────
    if n >= 4:
        first_half_avg  = sum(vols[:n//2]) / (n//2)
        second_half_avg = sum(vols[n//2:]) / (n - n//2)
        vol_change = (second_half_avg - first_half_avg) / first_half_avg if first_half_avg > 0 else 0

        # Climax: last bar volume > 3x average
        is_climax = vols[-1] > avg_vol * 3.0

        if is_climax:
            vol_trend = "climax"
        elif vol_change > 0.25:
            vol_trend = "expanding"
        elif vol_change < -0.25:
    ...
    price_range_pct = price_range / min(closes) * 100 if min(closes) > 0 else 0

    if vol_trend == "climax" and price_rising:
        phase = "climax_top"          # blow-off top — expect reversal soon
    elif vol_trend == "climax" and not price_rising:
        phase = "exhaustion_bottom"   # panic selling ending — watch for bounce
    elif obv_div == "bullish" and vol_trend in ("normal", "expanding"):
        phase = "accumulation"        # smart money buying quietly
    elif obv_div == "bearish" and vol_trend in ("normal", "expanding"):
        phase = "distribution"        # smart money selling quietly
    elif price_range_pct < 4.0 and vol_trend == "contracting":
        phase = "coiling"             # tight range + low vol = big move coming
    elif price_rising and obv_div == "confirming":
        phase = "trending_up"         # healthy uptrend confirmed by volume
    elif not price_rising and obv_div == "neutral":
        phase = "trending_down"       # confirmed downtrend
    else:
        phase = "neutral"

    # ── 6. Prediction + Confidence ────────────────────────────
    pred_map = {
        "accumulation":       (f"Institutional buying vs retail selling — upward move likely 3-10 days", 70),
        "distribution":       (f"Smart money exiting — downward move likely 3-10 days", 68),
        "coiling":            (f"Energy compressing — breakout imminent, watch volume for direction", 55),
        "trending_up":        (f"Volume-confirmed uptrend — hold winners, buy dips near VWAP", 75),
        "trending_down":      (f"Volume-confirmed downtrend — avoid longs, wait for exhaustion", 72),
        "climax_top":         (f"Blow-off top on {vols[-1]/avg_vol:.1f}x volume — reversal likely soon", 78),
        "exhaustion_bottom":  (f"Panic selling exhausting ({vols[-1]/avg_vol:.1f}x vol) — watch for bounce", 65),
        "neutral":            (f"No clear volume signal — wait for confirmation", 35),
    }
    prediction, conf = pred_map.get(phase, ("No clear signal", 30))

    # Boost confidence if multiple signals align
    alignments = 0
    if obv_div in ("bullish", "confirming") and phase in ("accumulation", "trending_up"): alignments += 1
    if price_vs_vwap == "below" and phase == "accumulation": alignments += 1  # cheap + accumulating
    if buy_sell_ratio > 1.3 and phase in ("accumulation", "trending_up"): alignments += 1
    if buy_sell_ratio < 0.7 and phase in ("distribution", "trending_down"): alignments += 1
    conf = min(92, conf + alignments * 8)

    return {
        "phase":           phase,
        "obv_slope_norm":  obv_slope_norm,   # % of avg vol per day — + = accumulating
        "obv_divergence":  obv_div,
        "vwap_20d":        vwap_20d,
        "price_vs_vwap":   price_vs_vwap,
        "vwap_gap_pct":    vwap_gap_pct,     # +% = above fair value, -% = below
...
def get_chart_section():
    ...
    for sym in RULES["universe"]:
        ...
        # ── 3-4 Week Volume Profile (predictive) ──────────────
        try:
            vp = compute_volume_profile(bars, lookback=20)
            if vp.get("phase") and vp["phase"] != "unknown":
                ...
                vwap_note  = f"VWAP20=${vp['vwap_20d']:.2f}({vp['vwap_gap_pct']:+.1f}%)"
                bs_note    = f"B/S={vp['buy_sell_ratio']:.2f}"
                obv_note   = f"OBV_slope={vp['obv_slope_norm']:+.0f}%"
                pred_short = vp['prediction'][:60]
                lines.append(
                    f"    → VOL_PROFILE[20d]: {phase_icon} | "
                    f"{vwap_note} | {bs_note} | {obv_note} | "
                    f"vol={vp['vol_trend']} | conf={vp['conf']}%"
                )
                lines.append(f"    → FORECAST: {pred_short} (conf={vp['conf']}%)")
        except Exception:
            pass  # never break chart section
...
# Replaces Capitol Trades HTML scraper (unreachable on Railway).
# Uses Alpaca news + structured Grok search for STOCK_ACT filings.
# Same output format — analyze_politician_signals() unchanged.

# Politicians to weight heavily when seen in Grok results
PRIORITY_POLITICIANS = {
    "Pelosi": {"party": "D", "focus": "tech"},
    "McCormick": {"party": "R", "focus": "finance"},
    "Tuberville": {"party": "R", "focus": "commodities"},
    "Khanna": {"party": "D", "focus": "tech"},
    "Gallagher": {"party": "R", "focus": "china_tech"},
    "Collins": {"party": "R", "focus": "healthcare"},
    "Ossoff": {"party": "D", "focus": "tech"},
    "Schiff": {"party": "D", "focus": "tech"},
}


def get_politician_trades() -> tuple:
    """
    Fetch congressional stock trades via two reliable methods:
      1. Alpaca news search — catches major covered disclosures
      2. Structured Grok live search — finds recent STOCK_ACT filings
    Returns: (summary_text, trades_list)
    trades_list dicts need: ticker, action, politician, party, size, filed
    """
    trades   = []
    lines    = []
    universe = set(RULES.get("universe", []))

    # ── Method 1: Alpaca news headlines about congress trades ────
    try:
        universe_str = ",".join(list(universe)[:12])
        news_resp = alpaca(
            "GET", "/v1beta1/news",
            params={
                "symbols": universe_str,
                "limit":   20,
                "sort":    "desc",
            }
        )
        news_items = news_resp.get("news", []) if isinstance(news_resp, dict) else []
        congress_keywords = ["congress", "senator", "representative", "pelosi",
                             "stock act", "disclosure", "politician", "lawmakers"]
        for item in news_items:
            headline = item.get("headline", "").lower()
            summary  = item.get("summary", "").lower()
            combined = headline + " " + summary
            if any(kw in combined for kw in congress_keywords):
                # Extract tickers from this news item
                for sym in item.get("symbols", []):
                    if sym in universe:
                        # Detect buy vs sell from text
                        action = ("buy"  if any(w in combined for w in ["bought","purchased","buy","long"])
                                  else "sell" if any(w in combined for w in ["sold","sale","sell"])
                                  else "buy")  # default to buy (more actionable)
                        # Extract politician name if mentioned
                        pol_name = "Unknown"
                        for name in PRIORITY_POLITICIANS:
                            if name.lower() in combined:
                                pol_name = name
                                break
                        party = PRIORITY_POLITICIANS.get(pol_name, {}).get("party", "?")
                        filed = item.get("created_at", "")[:10]
                        trades.append({
                            "ticker":     sym,
                            "action":     action,
                            "politician": pol_name,
                        ...
                        lines.append(f"  {icon} [{party}] {pol_name}: {action.upper()} "
                                     f"{sym} (news: {item.get('headline','')[:60]})")
    except Exception as e:
        log(f"⚠️ Alpaca news congress search: {e}")

    # ── Method 2: Grok structured search for STOCK_ACT filings ──
    try:
        universe_str = ", ".join(list(universe)[:15])
        prompt = (
            f"Search for US congressional STOCK_ACT stock trade disclosures filed in the last 45 days. "
            f"Priority stocks to watch: {universe_str}. "
            f"Include Pelosi, McCormick, Tuberville, Khanna, and any other active traders. "
            f"For each trade found, include the actual filing date and source. "
            f"Return ONLY valid JSON: "
            f'{{"trades": ['
            f'{{"politician":"Full Name", "party":"D or R", '
            f'"ticker":"SYMBOL", "action":"buy or sell", '
            f'"size":"$15k-$50k", "filed":"YYYY-MM-DD", '
            f'"source":"URL or disclosure source"}}'
            f'], "summary": "1-line summary"}}'
        )
        raw    = ask_grok(prompt,
                    "Financial research assistant with web search access. "
                    "Search for real congressional stock disclosures only. "
                    "Never invent trades. Cite sources. ONLY valid JSON.")
        result = parse_json(raw)
        if result and isinstance(result.get("trades"), list):
            for t in result["trades"]:
                ...
                party  = t.get("party", "?")
                size   = t.get("size", "")
                # Only keep trades for stocks we actually care about
                if ticker and (ticker in universe or len(ticker) <= 5):
                    # Dedup: skip if already in trades list
                    already = any(
                        t2["ticker"] == ticker and t2["politician"] == pol
                        for t2 in trades
                    )
                    if not already:
                        trades.append({
                        ...
                        lines.append(f"  {icon} [{party}] {pol}: "
                                     f"{action.upper()} {ticker} {size} ({filed})")
    except Exception as e:
        log(f"⚠️ Grok congress trade search: {e}")
    ...
    if trades:
        ...
        if buy_counts:
            top = sorted(buy_counts.items(), key=lambda x: -x[1])[:5]
            log(f"🏛️ Congress trades: {len(trades)} found | Top buys: {top}")
        return "\n".join(lines) if lines else "  No congress trades this cycle", trades

    log("ℹ️ No congressional trade data this cycle")
...
def get_vix() -> float:
    """
    Fetch current VIX (fear index) from Yahoo Finance via Alpaca bars.
    Falls back to SPY volatility estimate if VIX unavailable.
    VIX > 25 = high fear / defensive mode
    VIX > 35 = extreme fear / cash only
    VIX < 15 = low fear / bullish regime
    """
    _VIX_CACHE = getattr(get_vix, '_cache', {})
    cached_time = _VIX_CACHE.get('time', 0)
    if time.time() - cached_time < 900:  # 15 min cache
        return _VIX_CACHE.get('vix', 20.0)

    try:
        bars = get_bars("VIXY", days=3)   # VIXY ETF as VIX proxy on Alpaca
        if bars and len(bars) >= 1:
            # VIXY ~= VIX/10 roughly; scale up
            vixy_price = float(bars[-1].get('c', bars[-1].get('close', 14.0)))
            vix_est = round(vixy_price * 0.95, 1)  # close approximation
            get_vix._cache = {'vix': vix_est, 'time': time.time()}
            return vix_est
    except Exception:
        pass

    # Fallback: estimate VIX from SPY daily range
    try:
        spy_bars = get_bars("SPY", days=5)
        if spy_bars and len(spy_bars) >= 2:
            # Average daily range as volatility proxy
            ranges = []
            for b in spy_bars[-3:]:
                h = float(b.get('h', b.get('high', 0)))
                l = float(b.get('l', b.get('low', 0)))
                c = float(b.get('c', b.get('close', 1)))
                if c > 0:
                    ranges.append((h - l) / c * 100)
            if ranges:
                avg_range = sum(ranges) / len(ranges)
                # Convert daily % range to VIX estimate (rough)
                vix_est = round(avg_range * 6.5, 1)
                get_vix._cache = {'vix': vix_est, 'time': time.time()}
                return vix_est
    except Exception:
        pass
...
def get_market_regime() -> dict:
    """
    Classify the current market regime for risk management.
    Returns dict with: regime, vix, spy_trend, description, rules
    
    Regimes:
      BULL_QUIET    — VIX < 15, SPY uptrend: full size, hold winners
      BULL_NOISY    — VIX 15-20, SPY uptrend: normal trading
      NEUTRAL       — VIX 20-25, mixed: reduce size 25%, tighter exits
      BEAR_VOLATILE — VIX 25-35, SPY downtrend: half size, tight stops
      CRASH_MODE    — VIX > 35: cash only, no new longs
    """
    _REG_CACHE = getattr(get_market_regime, '_cache', {})
    if time.time() - _REG_CACHE.get('time', 0) < 600:
        return _REG_CACHE.get('regime_data', {})

    vix = get_vix()
    spy_trend, spy_price, spy_sma50, spy_change = get_spy_trend()

    spy_above_sma50 = spy_price > spy_sma50 if spy_sma50 > 0 else True

    if vix > 35:
        regime = "CRASH_MODE"
        desc   = f"⛔ CRASH MODE (VIX={vix:.0f}) — Cash only. No new longs."
        size_mult  = 0.0
        stop_mult  = 0.7   # tighten stops on existing positions
        entry_ok   = False
    elif vix > 25:
        regime = "BEAR_VOLATILE"
        desc   = f"🔴 HIGH FEAR (VIX={vix:.0f}) — Half size. Tight stops. Defensive."
        size_mult  = 0.5
        stop_mult  = 0.8
        entry_ok   = False  # no new longs in high fear
    elif vix > 20:
        regime = "NEUTRAL"
        desc   = f"🟡 ELEVATED RISK (VIX={vix:.0f}) — Reduce size 25%. Quick exits."
        size_mult  = 0.75
        stop_mult  = 0.9
        entry_ok   = True
    elif vix > 15:
        regime = "BULL_NOISY"
        desc   = f"🟢 NORMAL (VIX={vix:.0f}) — Standard trading. Full rules apply."
        size_mult  = 1.0
        stop_mult  = 1.0
        entry_ok   = True
    else:
        regime = "BULL_QUIET"
        desc   = f"🚀 LOW FEAR (VIX={vix:.0f}) — Trending market. Size up on conviction."
        size_mult  = 1.2   # can size up slightly in quiet bull
        stop_mult  = 1.0
        entry_ok   = True

    # Override: never buy longs if SPY below 50-day SMA
    if not spy_above_sma50 and regime not in ("CRASH_MODE", "BEAR_VOLATILE"):
        regime  = "BEAR_VOLATILE"
        desc    = f"🔴 SPY BELOW 50SMA + VIX={vix:.0f} — Defensive mode."
        size_mult  = 0.5
        stop_mult  = 0.8
        entry_ok   = False
    ...
    data = {
        "regime":     regime,
        "vix":        vix,
        "spy_above_50sma": spy_above_sma50,
        "description": desc,
        "size_mult":  size_mult,   # multiply position size by this
        "stop_mult":  stop_mult,   # multiply stop tightness by this
        "entry_ok":   entry_ok,    # False = no new longs this cycle
    }
    get_market_regime._cache = {'regime_data': data, 'time': time.time()}
    return data



def get_entry_quality() -> dict:
    """
    Score the current time for trade entry quality (0-100).
    Best entries happen in specific market windows.
    
    Windows:
      09:45-10:15 ET — post-open direction established ← BEST
      10:30-11:30 ET — morning trend confirmation      ← GOOD
      14:00-14:30 ET — afternoon setup window          ← GOOD
      11:30-14:00 ET — lunch lull, low volume          ← AVOID
      15:30-16:00 ET — end-of-day whipsaw              ← AVOID
      Pre-market     — wide spreads, low liquidity     ← AVOID
    """
    now_et = datetime.now(ZoneInfo("America/New_York"))
    hour   = now_et.hour
    minute = now_et.minute
    total_min = hour * 60 + minute  # minutes since midnight

    # Convert to minutes since 9:30 open
    open_min = 9 * 60 + 30   # 9:30am
    close_min = 16 * 60      # 4:00pm
    mins_from_open = total_min - open_min

    # Pre-market (before 9:30)
    if total_min < open_min:
        return {"score": 20, "window": "pre_market",
                "label": "⚠️ Pre-market — wide spreads, avoid new entries",
                "ok": False}

    # After hours (after 4:00pm)
    if total_min >= close_min:
        return {"score": 15, "window": "after_hours",
                "label": "⚠️ After-hours — low liquidity, avoid entries",
                "ok": False}

    # First 15 min (9:30-9:45) — opening volatility
    if mins_from_open < 15:
        return {"score": 30, "window": "open_volatile",
                "label": "⚠️ Opening 15min — high volatility, wait for direction",
                "ok": False}

    # BEST: 9:45-10:15 (mins 15-45)
    if 15 <= mins_from_open <= 45:
        return {"score": 95, "window": "best_morning",
                "label": "✅ BEST WINDOW (9:45-10:15) — post-open direction clear",
                "ok": True}

    # GOOD: 10:15-11:30 (mins 45-120)
    if 45 < mins_from_open <= 120:
        return {"score": 80, "window": "morning_trend",
                "label": "✅ Good window (10:15-11:30) — morning trend active",
                "ok": True}

    # LUNCH LULL: 11:30-14:00 (mins 120-270)
    if 120 < mins_from_open <= 270:
        return {"score": 45, "window": "lunch_lull",
                "label": "🟡 Lunch lull (11:30-2:00) — low volume, wider spreads",
                "ok": True}  # ok but lower conviction

    # GOOD: 14:00-14:30 (mins 270-300)
    if 270 < mins_from_open <= 300:
        return {"score": 85, "window": "afternoon_setup",
                "label": "✅ Good window (2:00-2:30) — afternoon trend confirmation",
                "ok": True}

    # 14:30-15:30 (mins 300-360)
    if 300 < mins_from_open <= 360:
        return {"score": 65, "window": "mid_afternoon",
                "label": "🟡 Mid-afternoon — OK if strong signal",
                "ok": True}

    # AVOID: Last 30 min (15:30-16:00)
    return {"score": 25, "window": "end_of_day",
            "label": "⚠️ Last 30min — end-of-day whipsaw, avoid new entries",
            "ok": False}



def check_portfolio_correlation(new_symbol: str, existing_symbols: list) -> dict:
    """
    Check if adding a new position creates too much correlation.
    Prevents holding 3 positions that all crash together.
    
    Sector groups (correlated moves):
      tech_semis: NVDA, AMD, INTC, QCOM, AVGO, MRVL
      ev_auto:    TSLA, RIVN, LCID, F, GM
      ai_cloud:   MSFT, GOOGL, META, AMZN, AAPL
      crypto_adj: MSTR, COIN, MARA, RIOT
      crypto:     BTC, ETH, SOL, any crypto
    """
    SECTOR_MAP = {
        # Tech/Semis
        "NVDA": "tech_semis", "AMD": "tech_semis", "INTC": "tech_semis",
        "QCOM": "tech_semis", "AVGO": "tech_semis",
        # EV/Auto
        "TSLA": "ev_auto", "RIVN": "ev_auto", "LCID": "ev_auto",
        "F": "ev_auto", "GM": "ev_auto",
        # AI/Cloud
        "MSFT": "ai_cloud", "GOOGL": "ai_cloud", "META": "ai_cloud",
        "AMZN": "ai_cloud", "AAPL": "ai_cloud",
        # Crypto-adjacent stocks
        "MSTR": "crypto_adj", "COIN": "crypto_adj",
        "MARA": "crypto_adj", "RIOT": "crypto_adj",
    }
    # Any crypto symbol gets "crypto" sector
    CRYPTO_SUFFIXES = ("USDT", "USD", "BTC", "ETH", "SOL")

    def get_sector(sym):
        if any(sym.endswith(s) for s in CRYPTO_SUFFIXES) or sym in ("BTC","ETH","SOL","DOGE","FET"):
            return "crypto"
        return SECTOR_MAP.get(sym, "other")

    new_sector   = get_sector(new_symbol)
    same_sector  = [s for s in existing_symbols if get_sector(s) == new_sector]
    total_corr   = len(same_sector)

    # Also flag if portfolio is already all "risk-on" with no diversification
    all_sectors  = list(set(get_sector(s) for s in existing_symbols))

    if total_corr >= 2:
        return {
            "ok": False,
            "warning": f"⚠️ CORRELATION BLOCK: Already hold {same_sector} in same sector ({new_sector}). Adding {new_symbol} = 100% correlated portfolio.",
            "same_sector_count": total_corr,
        }
    elif total_corr == 1:
        return {
            "ok": True,
            "warning": f"🟡 CORRELATION NOTE: {same_sector[0]} already held in {new_sector} sector. Both will move together in macro events.",
            "same_sector_count": total_corr,
        }
    else:
        return {
...
def reset_intraday_buys_if_new_day():
    ...
    # Reset crypto daily counters
    shared_state["crypto_stops_today"]  = 0
    shared_state["crypto_tp_today"]     = 0
    if stale:
...
def smart_sell(symbol, reason, pos):
    ...
    try:
        ...
        try:
            ...
            if snap_res.ok:
                ...
                if bid_price > 0 and ask_price > 0:
                    ...
                    # Clear any failed sell tracking on success
        ...
        log(f"❌ Sell {symbol}: {e}")
        err_str = str(e)
        if "403" in err_str or "Forbidden" in err_str or "halted" in err_str.lower():
            ...
            log(f"⚠️ {symbol} sell forbidden — attempt {fails}/3 (halted/restricted?)")
            if fails >= 3:
                # Mark as restricted so AI proposals are blocked
                ...
                log(f"🔒 {symbol} marked RESTRICTED — bot will no longer attempt sells")
                log(f"   ACTION NEEDED: Close {symbol} manually from Alpaca dashboard")
...
def check_exit_conditions(positions):
    ...
    for pos in positions:
        ...
        # ── THESIS INVALIDATION EXIT (early exit before hard stop) ────
        # If the reason for entering is gone, don't wait for -10%
        entry_reason = exit_cfg.get("entry_reason", "")
        days_held    = (datetime.now() - datetime.strptime(entry_date, "%Y-%m-%d")).days if entry_date != today else 0
        ...
        # Rule 1: HIGH FEAR market (VIX>25) + position losing → exit early at -5%
        if regime in ("BEAR_VOLATILE", "CRASH_MODE") and pnl_pct <= -0.05:
            thesis_broken = True
            thesis_reason = f"Market regime={regime}, VIX elevated, cutting loss early at {pnl_pct*100:.1f}%"

        # Rule 2: Time stop — position flat/losing after 3+ days → dead capital
        elif days_held >= 3 and pnl_pct <= 0.02 and strategy == "A":
            # But check if volume profile shows ACCUMULATION — if so, give it more time
            try:
                bars_tp = get_bars(symbol, days=30)
                vp_exit = compute_volume_profile(bars_tp, lookback=20) if bars_tp else {}
                if vp_exit.get("phase") in ("accumulation", "trending_up"):
                    # Volume says big money is still buying — override time stop once
                    log(f"   ℹ️ [{owner}] {symbol} time-stop OVERRIDE: vol_profile={vp_exit['phase']} (accumulating) — holding")
                else:
                    thesis_broken = True
                    thesis_reason = f"Position stuck {days_held} days P&L={pnl_pct*100:.1f}% vol_phase={vp_exit.get('phase','unknown')} — freeing capital"
            except Exception:
                thesis_broken = True
                thesis_reason = f"Position stuck {days_held} days with P&L={pnl_pct*100:.1f}% — freeing capital"

        # Rule 3: EV/auto stock (TSLA) on tariff-news day with >3% loss
        elif symbol in ("TSLA", "F", "GM", "RIVN", "LCID") and pnl_pct <= -0.03:
            vix_now = get_vix()
            if vix_now > 22:
                thesis_broken = True
                thesis_reason = f"{symbol} tariff-exposed stock down {pnl_pct*100:.1f}% in high-VIX regime"

        if thesis_broken:
            log(f"🧠 [{owner}] THESIS EXIT {symbol}: {thesis_reason}")
            if smart_sell(symbol, "thesis_exit", pos):
                record_trade("thesis_exit", symbol, pos.get("qty"), current_price,
                             float(pos.get("market_value", 0)), owner.lower(),
                             reason=thesis_reason, pnl_usd=pnl_usd,
...
def collaborative_session(equity, cash, positions, pos_symbols, open_count,
    ...
    # ── GROK INTEL FRESHNESS CHECK ───────────────────────────────
    # Detect if Grok is recycling stale intel (same narrative 3+ cycles)
    if grok_r1:
        try:
            r1_text = grok_r1 if isinstance(grok_r1, str) else str(grok_r1)
            # Extract key numbers/tickers as fingerprint
            import re as _re
            fingerprint = " ".join(_re.findall(r'\$[\d,]+M?|[+-]\d+\.\d+%|\b[A-Z]{2,5}\b', r1_text))[:200]
            history = shared_state.get("grok_intel_history", [])
            if len(history) >= 3:
                # Check if last 3 are identical fingerprints
                recent = [h.get("fp","") for h in history[-3:]]
                if all(fp == fingerprint for fp in recent):
                    log("⚠️ GROK INTEL STALE — same narrative 3 cycles in a row. Forcing fresh web search.")
                    # Force a new search on next cycle by clearing history
                    shared_state["grok_intel_history"] = []
                    shared_state["grok_last_changed"]  = None
                else:
                    shared_state["grok_last_changed"] = datetime.utcnow().isoformat()
            # Update history
            history.append({"fp": fingerprint, "time": datetime.utcnow().isoformat()})
            shared_state["grok_intel_history"] = history[-5:]  # keep last 5
        except Exception:
            pass
...
def execute_trades(final_trades, cash, pos_symbols, open_count, final_plan, features):
    ...
    for trade in final_trades:
        ...
        # ── COOLDOWN CHECK (2-loss streak) ──────────────────────
        cooldown = shared_state.get("cooldown_until")
        if cooldown:
            try:
                from datetime import datetime as _dt
                if _dt.now() < _dt.fromisoformat(cooldown):
                    log(f"⛔ COOLDOWN ACTIVE — skip {symbol} until {cooldown}")
                    continue
                else:
                    shared_state["cooldown_until"] = None  # expired
            except Exception:
                shared_state["cooldown_until"] = None

        # ── MARKET REGIME GATE (no new longs in CRASH/BEAR) ─────
        if action in ("buy", "long"):
            try:
                regime_data = get_market_regime()
                if not regime_data.get("entry_ok", True):
                    log(f"⛔ REGIME BLOCK {symbol}: {regime_data.get('description','')}")
                    continue
                # Apply regime size multiplier
                regime_mult = regime_data.get("size_mult", 1.0)
                notional = round(notional * regime_mult, 2)
                if regime_mult != 1.0:
                    log(f"📏 {symbol} size adjusted: {regime_mult:.0%} of {float(trade.get('notional_usd',0)):.2f} = ${notional:.2f} (regime)")
            except Exception:
                pass

            # ── CONVICTION SIZING (scale by confidence) ─────────
            try:
                streak_bonus = shared_state.get("win_streak_bonus", 1.0)
                # Scale: conf 90-100 → 1.25x, conf 75-79 → 0.75x
                if conf >= 90:
                    conv_mult = 1.25 * streak_bonus
                elif conf >= 80:
                    conv_mult = 1.0  * streak_bonus
                else:
                    conv_mult = 0.75
                conv_mult = round(min(1.5, max(0.5, conv_mult)), 2)
                if conv_mult != 1.0:
                    notional = round(notional * conv_mult, 2)
                    log(f"🎯 {symbol} conviction sizing: conf={conf}% → {conv_mult:.2f}x → ${notional:.2f}")
            except Exception:
                pass

            # ── PORTFOLIO CORRELATION CHECK ──────────────────────
            try:
                corr = check_portfolio_correlation(symbol, list(pos_symbols))
                if not corr.get("ok", True):
                    log(f"⚠️ {corr['warning']} — skipping {symbol}")
                    continue
                elif corr.get("warning"):
                    log(f"ℹ️ {corr['warning']}")
            except Exception:
                pass

            # ── ENTRY TIMING ADVISORY ────────────────────────────
            try:
                timing = get_entry_quality()
                if not timing.get("ok", True) and conf < 90:
                    log(f"⏰ TIMING SKIP {symbol}: {timing.get('label','')} (conf {conf}% < 90 override)")
                    continue
                elif not timing.get("ok", True):
                    log(f"⏰ TIMING WARN {symbol}: {timing.get('label','')} — high conf override")
            except Exception:
                pass
...
def run_cycle():
    ...
    try:
        ...
        _timing = get_entry_quality()
        shared_state["last_regime"] = _regime.get("regime", "NEUTRAL")
        log(f"🌡️ {_regime.get('description','')}")
        log(f"⏰ Entry timing: {_timing.get('label','')} (score={_timing.get('score',0)}/100)")
        if not _regime.get("entry_ok", True):
            log(f"🚫 NEW ENTRIES BLOCKED — {_regime.get('regime','')} regime")
    except Exception as _re:
        log(f"⚠️ Regime check error: {_re}")
...
def check_wake_conditions(cash, equity, positions, spy_change=0):
    ...
    try:
        ...
        # ── CRYPTO WAKE CONDITIONS ────────────────────────────────
        # Crypto runs 24/7 — these let it wake the AIs at night/weekends

        # 1. Crypto stop-loss fired 2+ times — position(s) exiting at loss
        crypto_stops = shared_state.get("crypto_stops_today", 0)
        if crypto_stops >= 2:
            return True, f"CRYPTO EMERGENCY — {crypto_stops} crypto stop-losses fired"

        # 2. Crypto wallet dropped >20% from day start — something went wrong
        crypto_wallet_now = shared_state.get("crypto_last_wallet_value", 0)
        crypto_day_start  = shared_state.get("crypto_day_start", 0)
        if crypto_wallet_now > 0 and crypto_day_start > 0:
            crypto_drawdown = (crypto_day_start - crypto_wallet_now) / crypto_day_start
            if crypto_drawdown >= 0.20:
                return True, (f"CRYPTO CRASH — wallet dropped "
                              f"{crypto_drawdown*100:.1f}% today "
                              f"(${crypto_day_start:.2f} → ${crypto_wallet_now:.2f})")

        # 3. Crypto take-profit hit — position closed profitably, USDT freed up
        crypto_tp = shared_state.get("crypto_tp_today", 0)
        if crypto_tp >= 1:
            # TP fired = USDT now available → AIs should review new entries
            crypto_usdt = shared_state.get("crypto_usdt_free", 0)
            if crypto_usdt >= 10:
                return True, (f"CRYPTO TP HIT — {crypto_tp} position(s) closed profitably, "
                              f"${crypto_usdt:.2f} USDT freed for new entries")
...
def run_autonomous_monitor(positions, pos_symbols, cash, equity):
    ...
    for pos in positions:
        ...
        # Quick volume profile check while bot runs autonomously
        _vp_note = ""
        try:
            _vp_bars = get_bars(symbol, days=30)
            if _vp_bars and len(_vp_bars) >= 10:
                _vp = compute_volume_profile(_vp_bars, lookback=20)
                _phase = _vp.get("phase", "")
                _bsr   = _vp.get("buy_sell_ratio", 1.0)
                if _phase in ("accumulation", "exhaustion_bottom"):
                    _vp_note = f" | 🟢 {_phase.upper()}(B/S={_bsr:.1f})"
                elif _phase in ("distribution", "climax_top"):
                    _vp_note = f" | 🔴 {_phase.upper()}(B/S={_bsr:.1f})"
                elif _phase == "coiling":
                    _vp_note = f" | 🟡 COILING(B/S={_bsr:.1f})"
        except Exception:
            pass

        log(f"   [{strategy}] {symbol}: {pnl_pct*100:+.2f}% (${pnl_usd:+.2f}) "
            f"peak=${peak_price:.2f}{_vp_note} | {ai_notes[:40] if ai_notes else ''}")
...
def trading_loop():
    ...
    shared_state["boot_unix"]  = time.time()
    log("⏱️  Boot time recorded — crypto starts in 2.5 min (staggered)")

    # ── Auto-sleep on boot if market is closed (weekend/overnight) ──
    # ai_sleeping defaults to False but shows misleading ✅ AWAKE on
    # snapshots when the market is closed and no cycle ever runs.
    # Set it True on boot during sleep hours so snapshot is accurate.
    _boot_mode, _ = get_market_mode()
    if _boot_mode == "sleep":
        shared_state["ai_sleeping"]  = True
        shared_state["sleep_reason"] = "market closed — bot monitoring only"
        log("😴 Market closed at boot — AIs starting in sleep mode")
    ...
    while True:
        try:
            ...
            if mode == "sleep":
                ...
                # ── Crypto emergency wake during sleep/weekend ────────────
                # Crypto runs 24/7 — check its wake conditions even when
                # stock market is closed so AIs can respond to emergencies.
                if shared_state.get("ai_sleeping"):
                    crypto_stops = shared_state.get("crypto_stops_today", 0)
                    crypto_tp    = shared_state.get("crypto_tp_today", 0)
                    crypto_now   = shared_state.get("crypto_last_wallet_value", 0)
                    crypto_start = shared_state.get("crypto_day_start", 0)
                    crypto_usdt  = shared_state.get("crypto_usdt_free", 0)
                    wake_crypto  = False
                    wake_reason  = ""
                    ...
                    if crypto_stops >= 2:
                        wake_crypto = True
                        wake_reason = f"CRYPTO EMERGENCY — {crypto_stops} stop-losses fired overnight"
                    elif crypto_now > 0 and crypto_start > 0:
                        drawdown = (crypto_start - crypto_now) / crypto_start
                        if drawdown >= 0.20:
                            wake_crypto = True
                            wake_reason = (f"CRYPTO CRASH — wallet down "
                                          f"{drawdown*100:.1f}% "
                                          f"(${crypto_start:.2f}→${crypto_now:.2f})")
                    elif crypto_tp >= 1 and crypto_usdt >= 10:
                        wake_crypto = True
                        wake_reason = (f"CRYPTO TP HIT — {crypto_tp} position(s) closed, "
                                      f"${crypto_usdt:.2f} USDT free for new entries")

                    if wake_crypto:
                        ai_wake(wake_reason)
                        log(f"🌅 CRYPTO WAKE during sleep mode — running full collaboration")
                        # Reset counters so we don't re-wake next tick
                        shared_state["crypto_stops_today"] = 0
                        shared_state["crypto_tp_today"]    = 0
                        run_cycle()
                ...
                if crypto_trader.is_enabled():
                    ...
                    if tradeable:
                        ...
                        for h in tradeable:
                            ...
                            free_qty    = h.get("free", qty)
                            free_val    = round(free_qty * px, 2) if px > 0 else 0
                            if lock > 0.0001:
                                _snap_lines.append(
                                    f"   {bot_tracked} {asset}: {free_qty:.4f} free = ${free_val:.2f} "
                                    f"@ ${px:.6f} | 🔒 {lock:.4f} STAKED (not tradeable)"
                                )
                            else:
                                _snap_lines.append(
...
if __name__ == "__main__":
    ...
    print(f"[STARTUP] Flask on 0.0.0.0:{port}", flush=True)
    log(f"🌐 Proxy on port {port}")
    t = threading.Thread(target=_delayed_trading_start, daemon=True)
    t.start()
