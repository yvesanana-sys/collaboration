...
def deploy_endpoint():
    ...
    if request.method == "GET":
        ...
        return jsonify({
            ...
            "files":        _DEPLOY_FILES,
            "how_to_deploy": "POST /deploy to push all files to GitHub",
            "setup_required": {} if configured else {
                "GITHUB_TOKEN": "Create at github.com/settings/tokens (repo scope)",
                "GITHUB_REPO":  "Your repo e.g. hanz/novatrade",
                "GITHUB_BRANCH": f"Branch (default: main, currently: {GITHUB_BRANCH})",
            }
        })
