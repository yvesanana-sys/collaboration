...
def smart_sell(symbol, reason, pos):
    ...
    # 4-method sell — tries each before giving up
    last_err = None
    ...
    # Method 4: Notional sell — handles fractional 403 restrictions
    try:
...
def run_premarket():
    ...
    # Reset self-repair counters for fresh day
    try:
        _repair_reset()
    except Exception:
        pass
...
Both AIs agree on this plan. Plain text 200 words."""
        ...
        # Always sleep after afterhours — fixes "AIs: AWAKE" showing all night
...
def check_wake_conditions(cash, equity, positions, spy_change=0):
    ...
    try:
        if not shared_state["ai_sleeping"]:
            return False, None
        thresholds = get_cash_thresholds(equity)
        # Only wake on a TRUE CROSSING — cash must have been below threshold before
