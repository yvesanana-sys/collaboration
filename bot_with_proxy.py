...
# Market data only needs RULES + log (no AI calls)
_market_data._set_context(RULES, log)
# Intelligence needs ask_grok + parse_json — injected after those are defined
...
# ── Late injection: intelligence needs AI functions ──────────
# Must happen AFTER ask_grok and parse_json are defined above
...
def check_wake_conditions(cash, equity, positions, spy_change=0):
    ...
    # Auto-clear stale restrictions — if position no longer held, un-restrict it
    try:
        restricted = shared_state.get("restricted_positions", set())
        if restricted:
            current_syms = set(pos_symbols) if hasattr(positions, '__iter__') else set()
