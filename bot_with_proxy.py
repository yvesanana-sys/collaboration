...
def dashboard():
    ...
    try:
        ...
        cwd_path = os.path.join(os.getcwd(), "dashboard.html")
        if os.path.exists(cwd_path):
            return send_file(cwd_path, mimetype="text/html")
        return f"<h2>NovaTrade Bot is running</h2><p>dashboard.html not found in {os.path.dirname(os.path.abspath(__file__))} or {os.getcwd()}</p><p><a href='/health'>Health check</a> | <a href='/stats'>Stats</a></p>", 200
    except Exception as e:
        return f"<h2>NovaTrade Bot running</h2><p>Dashboard error: {e}</p>", 200
...
def trading_loop():
    # Give Flask/gunicorn 5 seconds to bind port before heavy work starts
    # This prevents Railway health check timeouts on first deploy
    time.sleep(5)
...
if __name__ == "__main__":
    ...
    print(f"[STARTUP] Flask binding to 0.0.0.0:{port} (PORT env={os.environ.get('PORT','not set')})", flush=True)
