...
@app.route("/health")
def health():
    """Lightweight health check - no external calls, always responds instantly."""
    return jsonify({
        "status":   "ok",
        "bot":      BOT_NAME,
        "sleeping": shared_state.get("ai_sleeping", False),
        "uptime":   round(time.time() - shared_state.get("boot_unix", time.time()), 0),
    })
...
def dashboard():
    ...
    return jsonify({"error": "dashboard.html not found — add it to repo root"}), 404
...
# ── Start trading loop regardless of how Flask is launched ──────────
# Supports both: python bot_with_proxy.py  AND  gunicorn bot_with_proxy:app
