        ...
        if last_snap is None or (now_utc - last_snap).total_seconds() >= 900:
            ...
            try:
                ...
                if s_pos:
                    ...
                    for p in s_pos:
                        ...
                        strat    = shared_state.get("sleeping_strategies", {}).get(sym, {})
                        strat_lbl = strat.get("strategy", "A") if strat else "?"
                        _snap_lines.append(
            ...
            try:
                if crypto_trader.is_enabled():
                    ...
                    if c_pos:
                        _snap_lines.append(f"🪙 CRYPTO POSITIONS ({len(c_pos)}):")
                        for sym, pos in c_pos.items():
                            try:
                                cur_px  = pos.peak_price  # last known; live fetch optional
                                from binance_crypto import get_crypto_price as _gcp
                                cur_px  = _gcp(sym)
                            except Exception:
                                pass
                            ...
                            _snap_lines.append(
                                f"   {p_icon} {sym}  entry=${pos.entry_price:.6f} → now=${cur_px:.6f}  "
                                f"P&L={p_pnl:+.2f}%  stop=${pos.stop_price:.6f}  "
                                f"TP=${pos.tp_price:.6f}  held={hrs}h  qty={pos.qty}"
                            )
                    else:
                        _snap_lines.append("🪙 CRYPTO POSITIONS: none")
                else:
