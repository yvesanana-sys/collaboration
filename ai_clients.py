...
def ask_claude(prompt, system="You are a trading AI. Respond with ONLY valid compact JSON. No markdown, no prose, no extra text.", max_tokens=2400):
...
def parse_json(raw):
    try:
        ...
        try:
            ...
            # ── PROGRESSIVE TRUNCATION RECOVERY ──
            # Build candidate fixes from least-to-most aggressive.
            # Order matters — first parseable wins.
            candidates = []

            # 1. Bracket-balance fix: count [/] and {/} mismatches and
            #    append the right closers. Most common truncation pattern.
            #    First, strip any trailing junk after the last complete value
            #    (avoid lines like '{...},{"symb' where last token is partial).
            tail_clean = json_str.rstrip()
            # Drop trailing partial token: keep cutting until last char is
            # a known clean ending (close-bracket, close-quote, or digit/null/bool char).
            while tail_clean and tail_clean[-1] not in '}]"0123456789tfen':
                tail_clean = tail_clean[:-1].rstrip()
            # Also drop a trailing comma if present (it would break the close)
            tail_clean = tail_clean.rstrip(',').rstrip()
            # Now balance brackets
            need_close_arr = tail_clean.count("[") - tail_clean.count("]")
            need_close_obj = tail_clean.count("{") - tail_clean.count("}")
            if need_close_arr >= 0 and need_close_obj >= 0:
                candidates.append(tail_clean
                                  + ("]" * max(0, need_close_arr))
                                  + ("}" * max(0, need_close_obj)))

            # 2. Trim to last comma + close — handles mid-field truncation
            last_comma = json_str.rfind(",")
            if last_comma > 0:
                candidates.append(json_str[:last_comma] + "}")

            # 3. Close unclosed string + object (legacy fallback)
            candidates.append(
                json_str.rstrip().rstrip(',').rstrip('"') + '"}'
            )
