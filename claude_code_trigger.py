...
def trigger_claude_code_repair(error_msg: str, severity: str,
    ...
    with _trigger_lock:
        # Don't trigger if already repairing — but check for stale state
        state = _read_repair_state()
        if state.get("state") == "active":
            # Check if the active state is stale (> 10 minutes old = repair never completed)
            ...
            if updated_at:
                try:
                    ...
                    if age_sec < 600:  # Less than 10 min = genuinely active
                        is_stale = False
                        _log_trigger(f"⚠️  Repair already in progress ({state.get('repair_id', '')[:8]}) — skipping duplicate trigger")
                        return None
                    else:
                        _log_trigger(f"⚠️  Stale repair state detected (age {age_sec/60:.1f}m) — resetting and retrying")
                except Exception:
                    pass
            if is_stale:
                # Reset the stuck state
                _write_repair_state("idle", message="Auto-reset after timeout")
                _clear_repair_queue()
            # Fall through to create new repair
        ...
        # 3. Wake Claude Code SSH service FIRST to check if we can actually repair
        ...
        if woken:
            ...
            # Don't mark active if we can't wake the service — otherwise gets stuck
            _write_repair_state("idle", message=f"Wake failed: {error_msg[:60]}")
            _log_trigger(f"⚠️  Could not wake Claude Code SSH — staying idle, retry on next occurrence")
            # Clear queue since nothing will process it
            _clear_repair_queue()
