...
def get_live_asset_balance(symbol: str) -> float:
    ...
    try:
        ...
        _log(f"   ⚠️ get_live_asset_balance({symbol}) failed: {e}")
        return 0.0
...
def place_crypto_sell(symbol: str, qty: float,
    ...
    if live_balance > 0:
        if live_balance < qty:
            _log(f"   💡 {symbol}: requested sell qty {qty} > live balance {live_balance:.8f}")
            _log(f"      Using live balance (commission was deducted at buy)")
            qty = live_balance * 0.9995  # 0.05% safety buffer for rounding
    elif live_balance == 0:
        _log(f"   ⚠️ {symbol}: live balance = 0 — skipping sell (nothing to sell)")
...
def place_crypto_stop_market(symbol: str, qty: float,
    ...
    # ── Live balance check ──
    live_balance = get_live_asset_balance(symbol)
    if live_balance > 0 and live_balance < qty:
        _log(f"   💡 {symbol}: stop qty {qty} > live balance {live_balance:.8f} — using live balance")
        qty = live_balance * 0.9995
    elif live_balance == 0:
        _log(f"   ⚠️ {symbol}: live balance = 0 — cannot place stop")
...
class CryptoTrader:
    ...
    def _execute_exit(self, pos: CryptoPosition,
        ...
        try:
            ...
            # ── Handle ZERO_BALANCE — wallet is empty, clear ghost position ──
            if isinstance(result, dict) and result.get("error") == "ZERO_BALANCE":
                self._log(f"   🧹 {pos.symbol}: wallet empty — removing ghost position tracker")
