...
def get_crypto_24h_stats(symbol: str) -> dict:
    ...
    return {
        "symbol": symbol,
        "change_pct": float(data.get("priceChangePercent", 0)) / 100.0,
...
class CryptoTrader:
    """Main crypto trading engine for NovaTrade."""

    def __init__(self):
        """Initialize crypto trader state."""
        self.positions = {}  # Track open positions
        self.last_run = None

    def run_crypto_cycle(self, equity: float) -> dict:
        """
        Main entry point: run one crypto trading cycle.
        Called from bot_with_proxy.py trading loop.
        
        Returns: {"status": "ok", "trades_executed": N, "positions": {...}}
        """
