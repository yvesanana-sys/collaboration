    ...
    def run_exit_monitor(self) -> int:
        ...
        for symbol, pos in list(self.positions.items()):
            try:
                ...
                if exit_reason:
                    # ── v3.0: Require AI approval for crypto exits ────
                    # Crypto is volatile — a wick can hit the stop and recover
                    # in minutes. AI checks volume, BTC correlation, news
                    # before deciding. Bot only exits if AI explicitly approved
                    # OR if it is an absolute circuit-breaker level.
                    thesis_mgr = getattr(self, "_thesis_mgr", None)
                    if thesis_mgr:
                        may_sell, sell_reason = thesis_mgr.bot_may_sell(symbol)
                        if not may_sell:
                            # Not approved — wake the AI instead of auto-selling
                            self._log(f"   📋 {symbol}: exit triggered ({exit_reason}) "
                                      f"but AI approval required")
                            self._log(f"      Reason: {sell_reason[:60]}")
                            self._log(f"      Bot will NOT auto-exit — thesis conditions will wake AI")
                            # Update price history so thesis knows what happened
                            thesis = thesis_mgr.get_thesis(symbol)
                            if thesis:
                                thesis.update_price_history(current)
                            continue   # Skip exit — AI will decide on wake

                    result = self._execute_exit(pos, current, exit_reason)
                    if result:
                        exits += 1
                        # Clear thesis for this position after exit
                        if thesis_mgr:
                            thesis_mgr.clear_thesis(symbol)
            except Exception as e:
...
{f'GROK LIVE INTEL (from X/web search right now):{chr(10)}{grok_intel}' if grok_intel else ''}"""
    ...
    # ── v3.0 Wallet helpers (used by thesis_manager + wallet_intelligence) ──

    def get_total_wallet_value(self) -> float:
        """Return total crypto wallet value in USD."""
        if not self._enabled:
            return 0.0
        try:
            return get_full_wallet().get("total_value", 0.0)
        except Exception:
            return 0.0
    ...
    def get_usdt_free(self) -> float:
        ...
        if not self._enabled:
            return 0.0
        try:
            return get_full_wallet().get("usdt_free", 0.0)
        except Exception:
            return 0.0

    def get_wallet_holdings_list(self) -> list:
        """
        Return all holdings as list of dicts for thesis_manager + wallet_intelligence.
        Format: [{"symbol": "AUDIOUSDT", "value": 2.15, "price": 0.01635, "qty": 131.19}]
        """
        if not self._enabled:
            return []
        try:
            wallet   = get_full_wallet()
            all_h    = wallet.get("tradeable", []) + wallet.get("non_tradeable", [])
            return [
                {
                    "symbol": h.get("symbol", f"{h.get('asset','')}USDT"),
                    "asset":  h.get("asset", ""),
                    "qty":    h.get("free", 0),
                    "value":  h.get("value_usdt", 0),
                    "price":  h.get("price", 0),
                }
                for h in all_h
                if h.get("value_usdt", 0) >= 0.10 or h.get("qty", 0) > 0.000001
            ]
        except Exception:
            return []
