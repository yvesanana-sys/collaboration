...
{f'GROK LIVE INTEL (from X/web search right now):{chr(10)}{grok_intel}' if grok_intel else ''}"""
        ...
        if ENABLE_AI_COMPETITION:
            # Existing per-AI positions count against that AI's pool budget,
            # but the available USDT split is straightforward:
            claude_pool = round(crypto_pool * CLAUDE_POOL_PCT, 2)
            grok_pool   = round(crypto_pool * GROK_POOL_PCT,   2)
            self._log(f"   🥊 Pool split: Claude=${claude_pool:.2f} | "
                      f"Grok=${grok_pool:.2f} (of ${crypto_pool:.2f} USDT)")
        else:
            claude_pool = crypto_pool
            grok_pool   = crypto_pool
