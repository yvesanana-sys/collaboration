...
def place_crypto_buy(symbol: str, notional_usdt: float,
    ...
    """
    Place a MARKET buy order — always fills immediately at current price.
    LIMIT orders were causing filled=0 when market moved above limit price.
    Uses quoteOrderQty (spend exact USDT amount) — cleaner than qty-based.
    ...
    # ── Try quoteOrderQty first (spend exact USDT, Binance handles qty) ──
    # Round notional to 2 decimal places
    notional_str = f"{notional_usdt:.2f}"
    try:
        result = binance_post("/api/v3/order", {
            "symbol":        symbol,
            "side":          "BUY",
            "type":          "MARKET",
            "quoteOrderQty": notional_str,
        })
        # quoteOrderQty is not supported by all symbols — check for error
        if not result.get("error") and result.get("orderId"):
            return result
    except Exception:
        pass

    # ── Fallback: qty-based MARKET order ──────────────────────────────
    # Get live price to compute quantity
    try:
        live_price = limit_price if (limit_price and limit_price > 0) else get_crypto_price(symbol)
    except Exception:
        return {"error": f"Cannot get price for {symbol}"}

    if live_price <= 0:
        return {"error": f"Invalid price {live_price} for {symbol}"}

    raw_qty = notional_usdt / live_price
    if step_size > 0:
...
class CryptoTrader:
    ...
    def run_exit_monitor(self) -> int:
        ...
        Also auto-cancels stale unfilled buy orders (>30 min old).
        """
        if not self._enabled:
            return 0

        # ── Auto-cancel stale unfilled buy orders ─────────────
        # Any LIMIT buy sitting unfilled for >30 min is stale.
        # This happens when market moves away from limit price.
        # MARKET buys fill instantly so this only catches old LIMIT orders.
        try:
            ...
            for order in (all_open or []):
                ...
                order_time  = order.get("time", 0)  # ms since epoch
                ...
                if status != "NEW":
                    continue
                if filled_qty > 0:
                    continue  # partially filled — leave it alone

                # Check age
                age_mins = (time.time() * 1000 - order_time) / 60000

                # BUY orders: cancel after 30 min (price moved away from limit)
                # SELL orders: cancel after 60 min (much less common but can get stuck)
                stale_threshold = 30 if side == "BUY" else 60
                if age_mins < stale_threshold:
                    continue  # still fresh — give it time

                # Stale unfilled order — cancel it
                try:
                    cancel_crypto_order(symbol, order_id)
                    self._log(
                        f"   🗑️ Cancelled stale {side} order: {symbol} "
                        f"{orig_qty} @ ${price:.6f} | "
                        f"age={age_mins:.0f}min unfilled"
                    )
                except Exception as ce:
                    self._log(f"   ⚠️ Cancel failed for {symbol} order {order_id}: {ce}")
...
3. fee-aware rule: sell price must be > entry × 1.011 (fees + min profit)"""
        ...
        try:
            ...
            research_prompt = (
                f"Search Twitter/X and crypto news RIGHT NOW for:\n"
                f"1. Breaking news on: {', '.join(all_watch)}\n"
                f"2. Any coins trending/pumping on X in the last 2 hours\n"
                f"3. Whale movements, exchange listings, partnerships today\n"
                f"4. Macro crypto news (BTC ETF flows, regulation, Fed impact)\n"
                f"5. Market scan movers worth watching: "
                f"{[c['symbol'] for c in market_scan[:5]]}\n\n"
                f"Reply in plain text, 3-5 bullets MAX. Coin names + % + catalyst."
            )
            raw_intel = ask_grok_fn(
                research_prompt,
                "You are Grok with live Twitter/X and web access. "
                "Search NOW for breaking crypto news. Plain text bullets only, no JSON.",
            )
...
- STOP: {CRYPTO_RULES['stop_loss_pct']*100:.0f}% hard stop (crypto needs room — don't get shaken out)
- TP TARGET: 30-80% profit (not 5% — we need big wins to compound)
- TRAIL: activate trail at +{CRYPTO_RULES['trail_activate_pct']*100:.0f}%, trail {CRYPTO_RULES['trail_pct']*100:.0f}% from peak
                    ...
                    if holding and holding.get("free", 0) > 0:
                        ...
                        if curr > 0:
                            ...
                            if float(sell_price) <= 0:
                                ...
                                result = place_crypto_sell(sym, qty, curr * 1.001)
                        else:
            ...
            if crypto_pool < notional:
                ...
                for sell_sym in agreed_sells:
                    ...
                    try:
                        ...
                        if curr_price > 0:
                            # LIMIT sell — price rounded to exchange tick size
                            result = place_crypto_sell(sell_sym, qty, curr_price * 1.001)
                        else:
