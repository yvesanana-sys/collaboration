...
class CryptoTrader:
    ...
    # ── CRYPTO POOL SIZING ──────────────────────────────────
    def get_crypto_pool(self, total_equity: float) -> float:
        """
        How much USDT is reserved for crypto.
        30% of equity, but only what's actually in Binance.US account.
        """
        target = round(total_equity * CRYPTO_RULES["crypto_pool_pct"], 2)
        try:
            bal    = get_crypto_balance()
            usdt   = bal.get("usdt_free", 0)
            return min(usdt, target)
        except Exception:
            return 0.0
    ...
    def run_crypto_cycle(self, total_equity: float,
        ...
        try:
            ...
            if wallet.get("error"):
                ...
                self._log(f"   💵 USDT free: ${crypto_pool:.2f} | "
                          f"Total: ${wallet.get('total_value', 0):.2f}")
                if tradeable:
        ...
        # ── Situation classification via prompt_builder ───────
        situation_mode = "standard_monitoring"
        lessons_text   = ""
        if prompt_builder:
            try:
                # Classify situation using crypto-specific context
                pnl_pct = self.total_pnl / max(total_equity * 0.3, 1)
                ...
                situation_mode, focus, urgency, _ = prompt_builder.assess_situation(
                    equity      = total_equity,
                    cash        = crypto_pool,
                    positions   = [],   # crypto positions not in Alpaca format
                    spy_trend   = spy_trend,
                    pnl_today_pct = pnl_pct,
                    triple_syms = [],
                    projections = {},
                )
                self._log(f"   🧠 Crypto prompt mode: {situation_mode.upper().replace('_',' ')}")
                ...
                self._log(f"   ⚠️ Prompt builder context failed: {pe}")
...
SPY trend: {spy_trend.upper()} | Equity: ${total_equity:.2f}
{holdings_text}
CRYPTO RULES:
...
2. NEW BUYS: 1-2 best 2-3 day setups (HIGH conf + BULLISH + near proj_low + RSI<50)
3. AVOID: dangerous coins right now
