...
# Turtle math lives in turtle_math.py — single source of truth.
# projection_engine uses compute_atr internally to size projections.
from turtle_math import compute_atr


# ─────────────────────────────────────────────────────────────
# CORE COMPUTATION
# ─────────────────────────────────────────────────────────────
# NOTE: The Turtle math primitives (compute_atr, compute_donchian,
# compute_turtle_signal, compute_turtle_position_size) used to live
# here as well, but were moved to turtle_math.py to keep one source
# of truth. The Turtle gate in binance_crypto.py and bot_with_proxy.py
# imports them from turtle_math. If you need ATR/Donchian for use
# inside projection_engine itself, import from turtle_math:
#     from turtle_math import compute_atr, compute_donchian
...
def get_projection(symbol, bars, ind=None, open_price=None,
    ...
    # period=14 matches old in-file compute_atr default; Turtle uses period=20
    # by default, but projection_engine intentionally uses the shorter window.
