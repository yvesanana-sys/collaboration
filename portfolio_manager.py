...
def _set_context(log_fn, shared_state_ref, trade_history_ref,
    ...
    global log, shared_state, trade_history, RULES, alpaca, prompt_builder, binance_get
    log            = log_fn
...
RULES = {
    ...
    "max_positions":         3,      # Aggressive — up to 3 concurrent positions (was 2)
    # ── Stop / Profit targets (AGGRESSIVE PROFILE) ──────────
    # Moved from swing (20% TP) to aggressive scalping (8% TP)
    ...
    "interval_minutes":        5,
    # ── Exit strategies (AGGRESSIVE — faster turnover) ──────
...
# [boot sequence runs in bot_with_proxy.py after injection]
...
def record_trade(action, symbol, qty, price, notional, owner,
    ...
    try:
        ...
        # Sync Binance history after every trade to capture exchange fills
        import threading as _threading
        _threading.Thread(target=sync_binance_history, daemon=True).start()
    except Exception:
        pass
...
def _replay_trade_history_into_memory():
    ...
    Also performs a one-time backfill from Binance fill history so the
    AIs have a baseline of historical context on first ever boot.
    """
    global prompt_builder
    # ── Stock trade replay (existing logic) ─────────────────────
    if trade_history:
    ...
    # ── Binance fill backfill (NEW) ─────────────────────────────
    # Only runs once — backfilled flag prevents re-runs.
    try:
        binance_trades = _load_binance_history() or []
        if binance_trades and not getattr(prompt_builder.memory, "backfilled", False):
            n = prompt_builder.memory.backfill_from_binance_history(binance_trades)
            if n > 0:
                log(f"🧠 Binance backfill: {n} historical fills → AI memory baseline "
                    f"(total: {prompt_builder.memory.total_closed} closes, "
                    f"{prompt_builder.memory.total_wins} wins)")
            else:
                log(f"🧠 Binance backfill: no matchable buy/sell pairs in "
                    f"{len(binance_trades)} historical fills")
    except Exception as e:
        log(f"⚠️ Binance backfill failed: {e}")

    # ── Memory status report ────────────────────────────────────
    try:
        mem = prompt_builder.memory
        if mem.total_closed > 0 or len(mem.lessons) > 0:
            wr = round(mem.total_wins / max(mem.total_closed, 1) * 100, 1)
            log(f"🧠 AI Memory ready: {len(mem.lessons)} lessons | "
                f"{mem.total_closed} closes ({wr}% win rate) | "
                f"{len(mem.symbol_memory)} symbols tracked | "
                f"saved to {mem.MEMORY_FILE}")
        else:
            log(f"🧠 AI Memory: empty — building from this point forward "
                f"(persists to {mem.MEMORY_FILE})")
    except Exception:
        pass

def get_trading_pool(equity):
    """
    Calculate fund pools using the unified wallet-scaling reserve rule.
    Combined wallet (stocks + crypto) determines reserve %:
      < $1k → 0% (full freedom for AIs)
      $1k–$21k → 10%–29% (scaling +1% per $1k)
      $21k+ → 30% cap

    Falls back to the legacy hardcoded RULES["growth_reserve_pct"] if
    binance_crypto can't be imported (defensive — should never happen
    on Railway, but protects local dev).
    """
    # ── Compute combined wallet (stock equity + crypto wallet value) ──
    combined = equity
    try:
        import binance_crypto as _bc
        try:
            wallet = _bc.get_full_wallet() or {}
            combined = equity + float(wallet.get("total_value", 0) or 0)
        except Exception:
            combined = equity   # Crypto fetch failed — use stocks alone
        reserve_pct = _bc.get_wallet_reserve_pct(combined)
    except Exception:
        # Fallback if binance_crypto isn't loaded yet (boot ordering edge case)
        reserve_pct = RULES.get("growth_reserve_pct", 0.15)

    trading_pct = max(0.0, 1.0 - reserve_pct)
    reserve = equity * reserve_pct
    trading = equity * trading_pct
...
# Fetches real trade history from Binance.US, saves to volume.
# AI can learn from actual past trades, not just this session.
# ═══════════════════════════════════════════════════════════════

BINANCE_HISTORY_FILE = "/data/binance_trade_history.json"

_BINANCE_SYMBOLS = [
    "BTCUSDT","ETHUSDT","SOLUSDT","DOTUSDT","LTCUSDT","AVAXUSDT",
    "DOGEUSDT","ADAUSDT","LINKUSDT","ALGOUSDT","NEARUSDT","KAVAUSDT",
    "ATOMUSDT","FETUSDT","AUDIOUSDT","RVNUSDT","SHIBUSDT","XRPUSDT",
]


def _load_binance_history() -> list:
    """Load saved Binance trade history from volume."""
    for path in [BINANCE_HISTORY_FILE, "./binance_trade_history.json"]:
        try:
            with open(path) as f:
                data = json.load(f)
            log(f"📂 Binance history loaded: {len(data)} trades")
            return data
        except Exception:
            continue
    return []
...
def _save_binance_history(history: list):
    """Save Binance trade history to volume."""
    for path in [BINANCE_HISTORY_FILE, "./binance_trade_history.json"]:
        try:
            with open(path, "w") as f:
...
def fetch_binance_trade_history(days_back: int = 180) -> list:
    """
    Fetch last N days of Binance.US trade history across all symbols.
    Uses /api/v3/myTrades — returns all fills including past sessions.
    """
    if not binance_get:
        log("⚠️ Binance history: binance_get not injected yet")
        return []
    ...
    for symbol in _BINANCE_SYMBOLS:
        try:
            # /api/v3/myTrades is a SIGNED endpoint — requires HMAC + recvWindow
            # Without signed=True every call silently fails and returns 0 trades.
            resp = binance_get("/api/v3/myTrades", {
                "symbol":     symbol,
                "startTime":  cutoff_ms,
                "limit":      1000,
                "recvWindow": 5000,
            ...
            if resp and isinstance(resp, list) and len(resp) > 0:
                for t in resp:
                    all_trades.append({
                        ...
                        "notional":   float(t.get("quoteQty", 0)),
                ...
                fetched_syms.append(f"{symbol}:{len(resp)}")
        except Exception as _e:
            # Log first 3 errors only — avoids spam if API key lacks scope
            if err_count < 3:
                log(f"   ⚠️ Binance history {symbol}: {str(_e)[:100]}")
            err_count += 1
            continue

    all_trades.sort(key=lambda x: x.get("time_ms", 0))

    if all_trades:
        log(f"📊 Binance history fetched: {len(all_trades)} trades "
            f"across {len(fetched_syms)} symbols ({days_back}d)")
        _save_binance_history(all_trades)
    else:
        log(f"📊 Binance history: no trades found in last {days_back} days")

    return all_trades


def sync_binance_history() -> list:
    """
    Merge existing saved history with new trades from Binance.
    Called on boot and after every trade.
    Deduplicates by order_id — never double-counts.
    """
    existing    = _load_binance_history()
    existing_ids = {t.get("order_id") for t in existing}

    if existing:
        last_ms   = max(t.get("time_ms", 0) for t in existing)
        from datetime import datetime, timezone
        days_back = max(1, int((datetime.now(timezone.utc).timestamp() * 1000 - last_ms) / 86400000) + 1)
        log(f"🔄 Syncing Binance history ({days_back}d since last trade)...")
    else:
        days_back = 180
        log(f"🔄 First Binance sync — fetching 6 months of history...")

    new_trades = fetch_binance_trade_history(days_back=days_back)
    ...
    for t in new_trades:
        if t.get("order_id") not in existing_ids:
            merged.append(t)
            existing_ids.add(t.get("order_id"))
            added += 1

    merged.sort(key=lambda x: x.get("time_ms", 0))

    if added > 0:
        _save_binance_history(merged)
        log(f"✅ Binance history: +{added} new trades ({len(merged)} total saved)")
    else:
        log(f"✅ Binance history: up to date ({len(merged)} trades)")

    return merged


def get_binance_history_stats() -> dict:
    """
    Return summary stats of Binance history for AI context + /history endpoint.
    """
    history = _load_binance_history()
    if not history:
        return {"total_trades": 0, "message": "No Binance history yet — sync pending"}
    ...
    for t in history:
        by_sym[t["symbol"]][t["side"] + "s"].append(t)

    symbol_stats = {}
    for sym, data in by_sym.items():
        if data["buys"]:
            avg_buy  = sum(t["price"] for t in data["buys"]) / len(data["buys"])
            avg_sell = (sum(t["price"] for t in data["sells"]) / len(data["sells"])
                        if data["sells"] else None)
            pnl_pct  = (round((avg_sell - avg_buy) / avg_buy * 100, 2)
                        if avg_sell else None)
            symbol_stats[sym.replace("USDT", "")] = {
                "buys":     len(data["buys"]),
                "sells":    len(data["sells"]),
                "avg_buy":  round(avg_buy, 4),
                "avg_sell": round(avg_sell, 4) if avg_sell else None,
                "pnl_pct":  pnl_pct,
            }
    ...
    return {
        ...
        "symbols_traded": len(by_sym),
        "symbol_stats":   symbol_stats,
        "first_trade":    history[0].get("time", "")[:10] if history else "",
        "last_trade":     history[-1].get("time", "")[:10] if history else "",
    }
